...
function CourtManagementPage() {
  ...
  const selectedCourt = selectedCourtDetails ?? courts.find((court) => court.id === selectedCourtId) ?? null
                ...
                <tbody>
                  {courts.map((court) => (
                    ...
                    >
                      ...
                      <td>
                        <div className="admin-courts-actions">
                          <button className="app-button-secondary" type="button" onClick={() => openDetailsModal(court.id)}>
                            <Building2 size={16} /> View
                          </button>
                          <button className="app-button-secondary" type="button" onClick={() => openEditModal(court)}>
                            <Pencil size={16} /> Edit
                          </button>
                          <button
                            type="button"
      ...
      {isDeleteModalOpen ? (
        <div className="admin-modal-backdrop app-modal-backdrop" role="presentation" onClick={() => setIsDeleteModalOpen(false)}>
          <section className="admin-modal admin-modal--compact app-modal app-modal-compact" aria-label="Delete confirmation" onClick={(event) => event.stopPropagation()}>
            <div className="admin-modal__header app-modal-header">
              <div>
                <span className="app-eyebrow">Delete Court</span>
            ...
            <div className="admin-modal__actions">
              ...
              <button type="button" className="admin-danger-button app-button-danger" onClick={handleDeleteCourt} disabled={isSaving}>
                {isSaving ? 'Deleting...' : 'Delete Court'}
              </button>
            </div>
          </section>
        </div>
